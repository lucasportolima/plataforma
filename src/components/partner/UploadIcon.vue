    ...
    <b-field>
      <b-upload v-model="image" accept="image/*">
        <section class="input-logo">
          <img
            class="logo" v-if="partner.icon_url"
            :src="partner.icon_url" :alt="partner.name"
          >
          <i class="icon-ic_plus" v-if="!partner.icon_url" />
          <i class="icon-ic_edit" v-if="partner.icon_url" />
          <input type="file" @change="onFileChange" accept="image/*">
        </section>
      </b-upload>
    </b-field>
    <p class="upload-logo_infos">
      {{ $t("upload.upload-icon-info") }}
    </p>

    <modal-template @close="show = false" v-show="show">
      <template slot="header">
        <h3>Adicionar ícone</h3>
...
export default {
  ...
  methods: {
    ...
    async uploadImage (image) {
      try {
        await this.setPartnerIcon(image)
      } catch (error) {
        Event.publish(
